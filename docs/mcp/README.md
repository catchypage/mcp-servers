...
Поиск и случайный выбор фильмов/сериалов (TMDB через сервер), виджет в ChatGPT. Публичный тулы: **`find_movie`**. Домен: **`nuova.pro`** → **`POST https://nuova.pro/api/mcp`**. Тексты для каталога, CSP/аннотации, тест-кейсы: **[../moviepick/](../moviepick/)**.
