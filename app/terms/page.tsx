...
import { getTermsDocument } from '@/utils/legal/terms-content'
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    description: `Terms for using ${brand.name}, including ChatGPT integration.`,
  }
}
...
export default async function TermsPage() {
  ...
  const host = h.get('host') ?? h.get('x-forwarded-host') ?? 'localhost'
  const brand = getBrandByDomain(host)
  const doc = getTermsDocument(brand, host)
    ...
    <div className="min-h-screen company-bg-primary pt-24 pb-16">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav className="mb-8 text-sm company-text-secondary">
          ...
          <span className="company-text-primary">Terms of Service</span>
        </nav>
